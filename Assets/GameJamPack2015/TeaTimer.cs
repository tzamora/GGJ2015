...
public class LoopHandler
{
	...
	public void NextLoopWait(float time)
	{
		waitTime = time;
	}
}
...
public static class TeaTimer
{
    ...
    private static IEnumerator ExecuteInfiniteLoop(Action<LoopHandler> callback)
    {
		...
		loopHandler.timeSinceStart = 0f;

        while (true)
        {
			loopHandler.timeSinceStart += Time.deltaTime;

			if(loopHandler.waitTime > 0f)
			{
				yield return new WaitForSeconds(loopHandler.waitTime);
				
				loopHandler.timeSinceStart += loopHandler.waitTime;
				
				loopHandler.waitTime = 0f;
			}
