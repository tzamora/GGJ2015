...
public class StompEnemyController : MonoBehaviour {
	...
	{
		MeshRenderer[] meshRenderList = gameObject.GetComponentsInChildren<MeshRenderer> ();
		...
		.ttAppend (delegate() {
			...
			for (int i = 0; i < meshRenderList.Length; i++) {
				...
				meshRenderList[i].material = damageMaterial;
		...
		.ttAppend(delegate(){
			...
			for (int i = 0; i < meshRenderList.Length; i++) {
				...
				meshRenderList[i].material = normalMaterial;
